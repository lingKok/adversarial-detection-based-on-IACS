...
from unsupervised_detect import cos_sim_in_class_global


def get_feature_iacs(adv_batch, baseline_batch, adv_labels, baseline_labels, batch_size=1000):
    batch_num = int(np.ceil(adv_batch.shape[0] / float(batch_size)))

    feature = None
    for i_batch in range(batch_num):
        start = i_batch * batch_size
        end = np.minimum((i_batch + 1) * batch_size, len(adv_batch))
        adv_data = adv_batch[start:end]
        adv_label = adv_labels[start:end]
        baseline_data = baseline_batch[start:end]
        baseline_labels = baseline_labels[start:end]
        X_mean = np.mean(baseline_batch, 0)
        n_feed = end - start
        feature_batch1 = np.zeros(shape=(n_feed, 1))

        feature_batch1 = cos_sim_in_class_global(adv_data - X_mean, adv_label, baseline_data - X_mean, baseline_labels)

        if feature is not None:
            adv_feature = np.concatenate((adv_feature, feature_batch1), 0)

        else:
            adv_feature = feature_batch1
...
class CW2Attack(Attack):
    ...
    def __init__(self, model=None, dataset=None, kappa=0, init_const=0.001, lr=0.02, binary_search_steps=5,
                 max_iters=10000, lower_bound=0.0, upper_bound=1.0):
        """
    ...
    def get_iacs(self, perturbed_images, ys_targets):
        import os
        output = self.model(perturbed_images)
        layers_data_baseline_path = '../unsupervise/{}_layers_baseline.npy'.format(self.dataset)
        # layers_data_baseline, labels_baseline = np.load(layers_data_baseline_path, allow_pickle=True)
        # print(layers_data_baseline_path)
        if os.path.isfile(layers_data_baseline_path):
            layers_data_baseline, labels_baseline = np.load(layers_data_baseline_path,allow_pickle=True)
        else:
            print('please conduct the unsupervised_detect.py to generate the baseline data!')
            exit()
        lengh = len(layers_data_baseline)
        data_baseline = layers_data_baseline[lengh-1]
        features = get_feature_iacs(output.cpu().detach().numpy(), data_baseline, ys_targets, labels_baseline)
        return features
    ...
    def perturbation(self, samples, ys_targets, batch_size, device):
        ...
        for search_for_c in range(self.binary_search_steps):
            ...
            for iteration_times in range(self.max_iterations):
                ...
                l2dist = torch.sum((perturbed_images - (torch.tanh(var_samples) * half_range + mid_point)) ** 2,
                                   [1, 2, 3])
                ...
                iacs_loss = self.get_iacs(perturbed_images, ys_targets)
                loss_f = var_const * constraint_loss
                loss = l2dist.sum() + loss_f.sum() - iacs_loss.sum()  # minimize |r| + c * loss_f(x+r,l)
                optimizer.zero_grad()
                ...
                for i, (dist, score, img) in enumerate(
                        zip(l2dist.data.cpu().numpy(), prediction.data.cpu().numpy(),
