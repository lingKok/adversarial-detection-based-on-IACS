...
def cos_sim_in_class_global(data, label, base, base_label):
    """
    input: shape is (n,m) n is batch_size,m is dimension
    output:shape is (n,n)
    """
...
def get_iacs_by_random(data, label1, adv_data, label2, batch_size, k=10):
    ...
    for i_batch in range(batch_num):
        ...
        lid_batch_adv = cos_sim_in_class_global(X_adv_act - X_mean, label2, X_act - X_mean, label1)
        if lid is not None:
            lid = np.concatenate((lid, lid_batch), 0)
            lid_adv = np.concatenate((lid_adv, lid_batch_adv), 0)
        else:
...
def get_ics_by_random(data, label1, adv_data, label2, batch_size, k=10):
    ...
    for i_batch in range(batch_num):
        ...
        lid_batch_adv = cos_sim_in_class_global(X_adv_act , label2, X_act , label2)
        if lid is not None:
            lid = np.concatenate((lid, lid_batch), 0)
            lid_adv = np.concatenate((lid_adv, lid_batch_adv), 0)
        else:
...
def get_score_by_KNN(dict1, dict2):
    ...
    for i in range(dim):
        ...
        print('feature1 is :', len(feature1))
        feature2 = dict2[i].detach().numpy()
        print('feature2 is :', len(feature2))
...
def main():
    ...
    parse.add_argument('--type',default='kd',choices=['kd','lid','iacs','lacs','ilacs'])
    ...
    None_feature()
    adv_label = predict(raw_model, advloader,device)[0:1000]
    ...
    if not os.path.exists(feature_path):
        ...
        test_score, adv_score = np.load(feature_path)
    print('test score is :', test_score[0:5])
    print('adv score is:',adv_score[0:5])
